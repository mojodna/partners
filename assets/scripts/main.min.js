...
function getPrimaryStats(primaryhash) {
  var url = 'https://osmstats.redcross.org/hashtags/' + primaryhash + '/users';
...
function makePlaceholderProject(projectId, projectOrder) {
  // Adds error title
  $('ul li:nth-child(' + projectOrder + ') .HOT-Title p').html('<i class="ico icon collecticon-sign-danger"></i>\n<b>HOT Project #' + projectId + ' Not Active/Not Found in HOT Tasking Manager</b>');
  ...
  var ghIssueTitle = 'HOT Tasking Manager endpoint failure in ' + PT.mainHashtag + ' partner page';
...
function getUserActivityStats(hashtag) {
  ...
  var url = 'https://osmstats.redcross.org/top-users/' + hashtag;
  ...
  $.getJSON(url, function (userData) {
    ...
    var bldngSum = Object.keys(userData).map(function (user) {
      var bldngEdits = Math.round(Number(userData[user].buildings));
    ...
    var roadsSum = Object.keys(userData).map(function (user) {
      var roadsEdits = Math.round(Number(userData[user].road_kms));
...
function getGroupActivityStats(hashtags) {
  ...
  var url = 'https://osmstats.redcross.org/group-summaries/' + hashtagsString;
  ...
  $.getJSON(url, function (hashtagData) {
    ...
    if ($.isEmptyObject(hashtagData)) {
      ...
      console.warn('WARNING >> None of the secondary hashtags contain any ' + 'metrics according to the Missing Maps endpoint at ' + 'https://osmstats.redcross.org/group-summaries/' + hashtagsString + '. The partner graphs will not be displayed.');
    } else {
      ...
      var _hashtags = Object.keys(hashtagData);

      var totalSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(Number(vals.building_count_add) + Number(vals.building_count_mod) + Number(vals.road_count_add) + Number(vals.road_count_mod) + Number(vals.waterway_count_add) + Number(vals.poi_count_add));
      ...
      var bldngSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(Number(vals.building_count_add) + Number(vals.building_count_mod));
          acc.push({ name: generateHashtagUrl(ht), value: sum });
        }
        return acc;
      }, []).sort(function (a, b) {
      ...
      // skipping over hashtags if there are no metrics
      var roadsSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(Number(vals.road_km_add) + Number(vals.road_km_mod));
          acc.push({ name: generateHashtagUrl(ht), value: sum });
        }
        return acc;
      }, []).sort(function (a, b) {
...
function checkHashtags(hashtags) {
  if (hashtags.length < 2) {
    console.warn('WARNING >> There are not enough secondary hashtags listed ' + 'in order to represent differences in contribution level ' + 'between partners. The partner graphs will not be displayed.');
    $('.Team-User-Container').css('display', 'none');
  }
}
